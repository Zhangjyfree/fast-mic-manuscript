...
theme_gm <- function() {
  theme_classic(base_size = 8, base_family = BMC_FONT) +
    theme(
      ...
      axis.title         = md(size = 7),
      axis.title.x       = md(size = 7), axis.title.y = md(size = 7, angle = 90),
...
pA <- ggplot(all_summ, aes(level_idx, n_viable, colour = system)) +
  ...
  annotate("text", x = 5.8, y = max(all_summ$n_viable) * 0.55,
           ...
           fontface = "bold", lineheight = 0.85, hjust = 0, family = BMC_FONT) +
...
pD <- ggplot(all_summ, aes(level_idx, net_benefit, colour = system)) +
  ...
  labs(title = "Net benefit least negative at specialty sugars",
       y = "Mean β̄ = (β<sub>A</sub> + β<sub>B</sub>)/2") +
  theme_gm() +
  theme(legend.position = "none")
...
fig3 <- (pA + pB) / (pC + pD) +
  ...
  theme(plot.tag = element_text(face = "bold", size = 10, family = BMC_FONT))
