#!/usr/bin/env Rscript
# =============================================================================
# plot_figS6_benefit_landscape.R   (Supplementary Figure S3)
# -----------------------------------------------------------------------------
...
theme_gm <- function() {
  theme_classic(base_size = 7, base_family = BMC_FONT) +
    theme(
      plot.title       = md(face = "bold", size = 7.5, hjust = 0),
      strip.text       = md(face = "bold", size = 6),
      strip.text.x     = md(face = "bold", size = 6),
      strip.text.y     = md(face = "bold", size = 6, angle = -90),
...
figS6 <- ggplot(dat, aes(ba, bb)) +
  geom_bin2d(bins = 40) +
  quad_lines +
  facet_grid(system ~ short, labeller = sp_labeller) +
  ...
  labs(title = "**Benefit landscape** β<sub>A</sub> **vs** β<sub>B</sub> **across the gradient**",
       x = "β<sub>A</sub> (probiotic relative benefit)",
       y = "β<sub>B</sub> (commensal relative benefit)") +
  theme_gm()

ggsave(file.path(OUTDIR, "figS6_benefit_landscape.tiff"), figS6,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI,
       compression = "lzw", device = "tiff")
ggsave(file.path(OUTDIR, "figS6_benefit_landscape.pdf"), figS6,
       width = FIG_W, height = FIG_H, device = cairo_pdf)
ggsave(file.path(OUTDIR, "figS6_benefit_landscape.png"), figS6,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI)

cat("Saved figS6_benefit_landscape (TIFF/PDF/PNG) to", OUTDIR, "\n")
