#!/usr/bin/env Rscript
# =============================================================================
# plot_figS4_crossfeed_landscape.R   (Supplementary Figure S4)
# -----------------------------------------------------------------------------
...
figS4 <- ggplot(top_cf, aes(frac, met_key, fill = sys_label)) +
  geom_col(width = 0.78) +
...
ggsave(file.path(OUTDIR, "figS4_crossfeed_landscape.tiff"), figS4,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI,
       compression = "lzw", device = "tiff")
ggsave(file.path(OUTDIR, "figS4_crossfeed_landscape.pdf"), figS4,
       width = FIG_W, height = FIG_H, device = cairo_pdf)
ggsave(file.path(OUTDIR, "figS4_crossfeed_landscape.png"), figS4,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI)

cat("Saved figS4_crossfeed_landscape (TIFF/PDF/PNG) to", OUTDIR, "\n")
