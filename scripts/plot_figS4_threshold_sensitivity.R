#!/usr/bin/env Rscript
# =============================================================================
# plot_figS4_threshold_sensitivity.R   (Supplementary Figure S4)
# -----------------------------------------------------------------------------
...
figS4 <- pA + pB + plot_layout(nrow = 1) &
  theme(plot.tag = element_text(face = "bold", size = 9, family = "Arial"))

ggsave(file.path(OUTDIR, "figS4_threshold_sensitivity.tiff"), figS4,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI,
       compression = "lzw", device = "tiff")
ggsave(file.path(OUTDIR, "figS4_threshold_sensitivity.pdf"), figS4,
       width = FIG_W, height = FIG_H, device = cairo_pdf)
ggsave(file.path(OUTDIR, "figS4_threshold_sensitivity.png"), figS4,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI)

cat("Saved figS4_threshold_sensitivity (TIFF/PDF/PNG) to", OUTDIR, "\n")
