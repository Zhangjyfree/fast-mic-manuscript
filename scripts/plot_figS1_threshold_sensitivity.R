#!/usr/bin/env Rscript
# =============================================================================
# plot_figS1_threshold_sensitivity.R   (Supplementary Figure S1)
# -----------------------------------------------------------------------------
...
figS1 <- pA + pB + plot_layout(nrow = 1) &
  theme(plot.tag = element_text(face = "bold", size = 9, family = "Arial"))

ggsave(file.path(OUTDIR, "figS1_threshold_sensitivity.tiff"), figS1,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI,
       compression = "lzw", device = "tiff")
ggsave(file.path(OUTDIR, "figS1_threshold_sensitivity.pdf"), figS1,
       width = FIG_W, height = FIG_H, device = cairo_pdf)
ggsave(file.path(OUTDIR, "figS1_threshold_sensitivity.png"), figS1,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI)

cat("Saved figS1_threshold_sensitivity (TIFF/PDF/PNG) to", OUTDIR, "\n")
