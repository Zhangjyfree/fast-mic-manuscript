#!/usr/bin/env Rscript
# =============================================================================
# plot_figS8_threshold_sensitivity.R   (Supplementary Figure S5)
# -----------------------------------------------------------------------------
...
pA <- mk_panel(dfA, "μ<sub>min</sub>",
               "A  Viability-threshold sensitivity")
pB <- mk_panel(dfB, "ε<sub>β</sub>",
               "B  Benefit-threshold sensitivity")

figS8 <- pA + pB + plot_layout(nrow = 1) &
  theme(plot.tag = element_text(face = "bold", size = 9, family = BMC_FONT))

ggsave(file.path(OUTDIR, "figS8_threshold_sensitivity.tiff"), figS8,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI,
       compression = "lzw", device = "tiff")
ggsave(file.path(OUTDIR, "figS8_threshold_sensitivity.pdf"), figS8,
       width = FIG_W, height = FIG_H, device = cairo_pdf)
ggsave(file.path(OUTDIR, "figS8_threshold_sensitivity.png"), figS8,
       width = FIG_W, height = FIG_H, dpi = FIG_DPI)

cat("Saved figS8_threshold_sensitivity (TIFF/PDF/PNG) to", OUTDIR, "\n")
